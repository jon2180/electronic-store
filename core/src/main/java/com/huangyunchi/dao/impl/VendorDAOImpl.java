...
public class VendorDAOImpl extends BaseDAO implements VendorDAO {
    ...
    public int save(Vendor vendor) throws SQLException {
        ...
        Long rst = getQr().insert(sql, new ScalarHandler<>(),
    ...
    public int remove(Integer id) throws SQLException {
        return getQr().update("delete from vendor where vendor_id=?", id);
    }

    @Override
    public Vendor findById(Integer id) throws SQLException {
        return getQr().query("select * from vendor where vendor_id=?", new BeanHandler<>(Vendor.class), id);
    }

    @Override
    public Vendor findByTel(String tel) throws SQLException {
        return getQr().query("select * from vendor where vendor_tel=?", new BeanHandler<>(Vendor.class), tel);
    }
}
